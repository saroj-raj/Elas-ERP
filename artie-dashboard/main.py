...
import os, sys, time, subprocess, signal, webbrowser, pathlib, shutil
...
VENV_PYTHON = BACKEND / ".venv" / "Scripts" / "python.exe" if on_windows() else BACKEND / ".venv" / "bin" / "python"
...
def ensure_python_deps():
    ...
    if missing:
        ...
        print("[launcher] Activate your backend venv and install:")
        print("  cd backend && python -m venv .venv && .\\.venv\\Scripts\\Activate.ps1  (PowerShell)")
...
def main():
    ...
    if not VENV_PYTHON.exists():
        print(f"[launcher] Backend venv not found at {VENV_PYTHON}")
        print("[launcher] Create it with:")
        print(f"  cd backend && python -m venv .venv && .\\.venv\\Scripts\\Activate.ps1")
        print("  pip install -r requirements.txt")
        sys.exit(1)
    ...
    if not npm_exe:
        ...
        print("[launcher] Backend will start, but you need to run frontend separately:")
        print("[launcher]   Open a new terminal and run:")
        print("[launcher]   cd frontend && npm install && npm run dev")
        skip_frontend = True
        input("[launcher] Press Enter to continue with backend only...")
    ...
    if not skip_frontend:
        ...
        web_proc = run([npm_exe, "run", "dev"], cwd=FRONTEND)
